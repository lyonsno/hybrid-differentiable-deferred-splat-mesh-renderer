...
export function buildPerPixelRetainedContributorTraces({
  ...
  const projectedByTileIndex = Array.isArray(projectedContributors)
    ? groupContributorsByTileIndex(projectedContributors)
    : null;
  const retainedByTileIndex = Array.isArray(retainedContributors)
    ? groupContributorsByTileIndex(retainedContributors)
    : null;
  const supportStatus = classifyRetainedPixelSupport(projectedContributors, retainedContributors);
  ...
  return anchors.map((anchorPixel) => {
    ...
    const projectedSources = projectedByTileIndex?.get(tileAddress.tileIndex) ?? [];
    const retainedSources = retainedByTileIndex?.get(tileAddress.tileIndex) ?? [];
    const projectedTraceEntries = projectedSources.map(projectedContributorToTraceEntry);
    const retainedTraceEntries = retainedSources.map(retainedContributorToTraceEntry);
    const droppedTraceEntries = summarizeDroppedContributors(projectedSources, retainedSources);
    const status = supportStatus === "still-blocked"
      ? supportStatus
      : classifyRetainedPixelSupport(projectedSources, retainedSources);

    return {
      status,
      anchorPixel: cloneAnchorPixel(anchorPixel),
      tileAddress,
      projectedContributors: projectedTraceEntries,
      retainedContributors: retainedTraceEntries,
      droppedContributors: droppedTraceEntries,
      traceRecord: {
...
export function classifyRetainedPixelSupport(projectedContributors, retainedContributors) {
  if (!Array.isArray(projectedContributors) || !Array.isArray(retainedContributors)) {
    return "still-blocked";
  }
  if (projectedContributors.length === 0) {
    return "absent";
  }
  if (retainedContributors.length === 0) {
    return "insufficient";
  }

  const retainedKeys = new Set(retainedContributors.map(contributorKey));
  const droppedContributors = projectedContributors.filter((contributor) => !retainedKeys.has(contributorKey(contributor)));
  if (droppedContributors.length === 0) {
    return "sufficient";
  }

  const projectedCoverage = sumFinite(projectedContributors, (contributor) => finiteNumber(contributor?.coverageWeight, 0));
  const retainedCoverage = sumFinite(retainedContributors, (contributor) => finiteNumber(contributor?.coverageWeight, 0));
  const retainedFraction = projectedCoverage > 0 ? retainedCoverage / projectedCoverage : 0;
  return retainedFraction >= 0.75 ? "misleadingly-sufficient" : "insufficient";
}

function summarizeDroppedContributors(projectedContributors, retainedContributors) {
  const retainedKeys = new Set(retainedContributors.map(contributorKey));
  return projectedContributors
    .filter((contributor) => !retainedKeys.has(contributorKey(contributor)))
    .map((contributor) => retainedContributorToTraceEntry({
      ...contributor,
      retentionStatus: "dropped",
      retained: false,
      overflowReason: contributor?.overflowReason ?? "perTileRetainedCap",
      retentionBand: contributor?.retentionBand ?? "middle",
    }));
}

function groupContributorsByTileIndex(contributors) {
...
function retainedContributorToTraceEntry(contributor) {
  return {
    splatIndex: positiveInteger(contributor?.splatIndex ?? 0, "splatIndex"),
    originalId: positiveInteger(contributor?.originalId ?? 0, "originalId"),
    retentionStatus: contributor?.retentionStatus === "dropped" ? "dropped" : "retained",
    retentionWeight: finiteNumber(contributor?.retentionWeight ?? contributor?.coverageWeight, 0),
    occlusionWeight: finiteNumber(contributor?.occlusionWeight ?? contributor?.coverageWeight, 0),
    overflowReason: contributor?.overflowReason ?? "none",
    retentionBand: normalizeRetentionBand(contributor?.retentionBand),
    retained: contributor?.retentionStatus === "dropped" ? false : true,
    coverageWeight: finiteNumber(contributor?.coverageWeight, 0),
    centerPx: normalizePair(contributor?.centerPx),
...
function normalizeRetentionBand(value) {
  return value === "front" || value === "middle" || value === "back" ? value : "middle";
}
...
function contributorKey(contributor) {
  return `${positiveInteger(contributor?.splatIndex ?? 0, "splatIndex")}:${positiveInteger(contributor?.originalId ?? 0, "originalId")}`;
}

function sumFinite(contributors, project) {
  let total = 0;
  for (const contributor of contributors) {
    total += project(contributor);
  }
  return total;
}
