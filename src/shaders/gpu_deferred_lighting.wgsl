...
fn main(@builtin(global_invocation_id) gid: vec3u) {
  ...
  let coverageAlpha = colorSample.a; // coverage confidence from compositor, preserved for DOF
  let depth = textureLoad(depthTexture, px, 0).r;

  // Background: pass through
  if (depth >= 0.9999) {
    textureStore(outputLit, px, vec4f(albedoSrgb, coverageAlpha));
    return;
  }
  ...
  textureStore(outputLit, px, vec4f(mapped, coverageAlpha));
}
