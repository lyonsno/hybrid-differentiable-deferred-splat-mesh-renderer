...
// Sorts (key, value) pairs where key = (mortonTileId << 16 | depth_u16).
...
// --- Histogram pass (scalar fallback) ---
...
fn scatter(
  ...
  for (var item = 0u; item < ITEMS_PER_THREAD; item++) {
    ...
    let wordIdx = localId.x >> 5u;
    let bitIdx = localId.x & 31u;
