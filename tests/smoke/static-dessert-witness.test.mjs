...
test("static dessert witness classifier separates transferred selected color from underpowered selected color", () => {
  const visualGapAnchors = [
    { id: "visual-gap-1", kind: "plate-covered-tile-local-missing", x: 640, y: 342, score: 71, plateDelta: 88, tileLocalDelta: 9 },
  ];
  const badPixelAnchors = [
        ...
        perPixelFinalColorAccumulation: [
          {
            ...
            finalColorAccumulation: {
              steps: [{
                ...
                sourceRole: "foreground",
                sourceFrontierAlphaSupport: "foreground-spatial-support",
                sourceFrontierSupportPixelWeight: 0.28,
                alphaTransferWeight: 0.72,
                colorTransferWeight: 0.58,
                coverageAlpha: 0.88,
                colorAlpha: 0.7,
                colorOcclusionAlpha: 0.78,
                sourceColor: [1, 0.9, 0.75],
                contributionColor: [0.7, 0.63, 0.525],
              }],
              outputColor: [0.16, 0.14, 0.11, 1],
              remainingTransmittance: 0,
            },
          },
        ],
  ...
  assert.equal(provenance.category, "selected-source-color-transferred-overoccluded");
  assert.equal(provenance.maxContributionLuma, 162.512);
  assert.equal(provenance.finalOutputLuma, 36.232);
  assert.equal(provenance.finalToContributionLumaRatio, 0.223);
  assert.equal(provenance.maxColorOcclusionGap, 0.08);
  assert.equal(provenance.maxColorAlpha, 0.7);
  assert.equal(provenance.colorTransferRatio, 0.806);
  assert.equal(
    result.metrics.operatorVisibleBadPixelClassification.rgbProvenanceCategoryCounts["selected-source-color-transferred-overoccluded"],
    1,
  );
});

test("static dessert witness classifier keeps bright final transferred color out of overoccluded bucket", () => {
  const visualGapAnchors = [
    { id: "visual-gap-1", kind: "plate-covered-tile-local-missing", x: 640, y: 342, score: 71, plateDelta: 88, tileLocalDelta: 9 },
  ];
  const badPixelAnchors = [
  ...
  const provenance = result.metrics.operatorVisibleBadPixelTrace.anchors[0].rgbProvenance;
  assert.equal(provenance.status, "classified");
  assert.equal(provenance.category, "selected-source-color-transferred-still-mismatched");
  assert.equal(provenance.maxContributionLuma, 162.512);
  assert.equal(provenance.finalOutputLuma, 134.564);
  assert.equal(provenance.finalToContributionLumaRatio, 0.828);
  assert.equal(provenance.maxColorOcclusionGap, 0.08);
  assert.equal(
    result.metrics.operatorVisibleBadPixelClassification.rgbProvenanceCategoryCounts["selected-source-color-transferred-still-mismatched"],
    1,
  );
});

test("static dessert witness classifier separates weak transferred color authority from generic transferred mismatch", () => {
  const visualGapAnchors = [
    { id: "visual-gap-1", kind: "plate-covered-tile-local-missing", x: 640, y: 342, score: 71, plateDelta: 88, tileLocalDelta: 9 },
  ];
  const badPixelAnchors = [
        ...
        perPixelFinalColorAccumulation: [
          {
            ...
            finalColorAccumulation: {
              steps: [{
                ...
                sourceFrontierAlphaSupport: "foreground-spatial-support",
                sourceFrontierSupportPixelWeight: 0.28,
                alphaTransferWeight: 1,
                colorTransferWeight: 0.38,
                coverageAlpha: 0.92,
                colorAlpha: 0.38,
                colorOcclusionAlpha: 0.44,
                sourceColor: [1, 0.9, 0.75],
                contributionColor: [0.38, 0.342, 0.285],
              }],
              outputColor: [0.62, 0.58, 0.5, 1],
              remainingTransmittance: 0,
            },
          },
        ],
  ...
  const provenance = result.metrics.operatorVisibleBadPixelTrace.anchors[0].rgbProvenance;
  assert.equal(provenance.status, "classified");
  assert.equal(provenance.category, "selected-source-color-transferred-weak-authority");
  assert.equal(provenance.maxSourceLuma, 232.16);
  assert.equal(provenance.maxContributionLuma, 88.221);
  assert.equal(provenance.colorTransferRatio, 0.38);
  assert.equal(provenance.maxColorOcclusionGap, 0.06);
  assert.equal(
    result.metrics.operatorVisibleBadPixelClassification.rgbProvenanceCategoryCounts["selected-source-color-transferred-weak-authority"],
    1,
  );
});

test("static dessert witness classifier separates late dull support suppression from generic transferred mismatch", () => {
  const visualGapAnchors = [
    { id: "visual-gap-1", kind: "plate-covered-tile-local-missing", x: 640, y: 342, score: 71, plateDelta: 88, tileLocalDelta: 9 },
  ];
  const badPixelAnchors = [
              ...
              steps: [
                {
                  ...
                  sourceColor: [0.9, 0.9, 0.9],
                  contributionColor: [0.38, 0.38, 0.38],
                  runningColor: [0.48, 0.48, 0.48],
                },
                {
                  splatIndex: 12,
                  viewRank: 12,
                  ...
                  coverageAlpha: 0.79,
                  colorAlpha: 0.28,
                  colorOcclusionAlpha: 0.53,
                  sourceFrontierColorAuthority: 0.65,
                  sourceColor: [0.39, 0.39, 0.39],
                  contributionColor: [0.11, 0.11, 0.11],
                  runningColor: [0.31, 0.31, 0.31],
                },
              ],
              outputColor: [0.31, 0.31, 0.31, 1],
              remainingTransmittance: 0,
            },
          },
        ],
  ...
  const provenance = result.metrics.operatorVisibleBadPixelTrace.anchors[0].rgbProvenance;
  assert.equal(provenance.status, "classified");
  assert.equal(provenance.category, "selected-source-color-transferred-late-support-suppressed");
  assert.equal(provenance.maxRunningLuma, 122.4);
  assert.equal(provenance.maxPostPeakRunningLumaDrop, 43.35);
  assert.equal(provenance.postPeakRunningLumaRatio, 0.646);
  assert.equal(provenance.maxPostPeakSupportColorOcclusionGap, 0.25);
  assert.deepEqual(provenance.peakRunningColorStep, {
    index: 0,
    splatIndex: 11,
    viewRank: 11,
    candidateSourceClassMask: 9,
    isSupportStep: false,
    sourceLuma: 229.5,
    contributionLuma: 96.9,
    runningLuma: 122.4,
    colorAlpha: 0.43,
    colorOcclusionAlpha: 0.43,
    colorOcclusionGap: 0,
    sourceFrontierColorAuthority: null,
  });
  assert.deepEqual(provenance.postPeakSuppressorStep, {
    index: 1,
    splatIndex: 12,
    viewRank: 12,
    candidateSourceClassMask: 8,
    isSupportStep: true,
    sourceLuma: 99.45,
    contributionLuma: 28.05,
    runningLuma: 79.05,
    colorAlpha: 0.28,
    colorOcclusionAlpha: 0.53,
    colorOcclusionGap: 0.25,
    sourceFrontierColorAuthority: 0.65,
    runningLumaDrop: 43.35,
  });
  assert.equal(
    result.metrics.operatorVisibleBadPixelClassification.rgbProvenanceCategoryCounts["selected-source-color-transferred-late-support-suppressed"],
    1,
  );
});

test("static dessert witness classifier separates late bright plate contamination from generic transferred mismatch", () => {
  const visualGapAnchors = [
    { id: "visual-gap-1", kind: "plate-covered-tile-local-missing", x: 640, y: 342, score: 71, plateDelta: 88, tileLocalDelta: 9 },
  ];
  const badPixelAnchors = [
  ...
  assert.deepEqual(provenance.peakRunningColorStep, {
    ...
    isSupportStep: true,
    sourceLuma: 245.127,
    contributionLuma: 127.664,
    runningLuma: 144.038,
    colorAlpha: 0.52,
    colorOcclusionAlpha: 0.67,
    colorOcclusionGap: 0.15,
    sourceFrontierColorAuthority: 1,
  });
  assert.equal(
...
test("static dessert witness classifier blocks operator-visible bad-pixel accusation until trace/canvas parity matches", () => {
  ...
  const result = classifyStaticDessertWitness({
    captures: [
      ...
      witnessCapture("operator-visible-bad-pixel-trace", {
        ...
        traceCanvasParity: traceCanvasParityForTest(badPixelAnchors, {
          status: "mismatch",
          sampledRgba8: [64, 64, 64, 255],
          deltaRgba8: [-173, -160, -137, 0],
          maxDelta: 173,
        }),
      }),
    ],
  ...
  assert.equal(result.closeable, false);
  assert.equal(result.metrics.operatorVisibleBadPixelTrace.status, "present");
  assert.equal(result.metrics.operatorVisibleBadPixelTrace.traceCanvasParity.status, "trace-canvas-mismatch");
  assert.equal(result.metrics.operatorVisibleBadPixelClassification.category, "trace-canvas-parity-blocked");
  assert.equal(result.metrics.operatorVisibleBadPixelClassification.stage, "trace-canvas-parity");
  assert.equal(
    result.findings.some((finding) => finding.kind === "operator-visible-bad-pixel-trace-canvas-parity-blocked"),
    true,
  );
  assert.equal(result.observations.visibleHoles.category, "conic-coverage-pressure");
});

test("static dessert witness classifier blocks operator-visible bad-pixel accusation from CPU final-trace parity fallback", () => {
  const visualGapAnchors = [
    { id: "visual-gap-1", kind: "plate-covered-tile-local-missing", x: 640, y: 342, score: 71, plateDelta: 88, tileLocalDelta: 9 },
  ];
  const badPixelAnchors = [
  ...
  const cpuFinalTraceParity = traceCanvasParityForTest(badPixelAnchors);
  cpuFinalTraceParity.comparisonClass = "exact-route-cpu-final-trace-vs-canvas";
  cpuFinalTraceParity.predictionSource = "cpu-final-trace";
  cpuFinalTraceParity.liveCompositorInputReadbackStatus = "missing";
  cpuFinalTraceParity.anchors = cpuFinalTraceParity.anchors.map((anchor) => ({
    ...anchor,
    predictionSource: "cpu-final-trace",
    liveCompositorRgba8: null,
  }));
  const result = classifyStaticDessertWitness({
    captures: [
      witnessCapture("final-color", {
      ...
      witnessCapture("operator-visible-bad-pixel-trace", {
        ...
        traceCanvasParity: cpuFinalTraceParity,
      }),
    ],
  });
  ...
  assert.equal(result.metrics.operatorVisibleBadPixelTrace.status, "present");
  assert.equal(result.metrics.operatorVisibleBadPixelTrace.traceCanvasParity.status, "trace-canvas-match");
  assert.equal(result.metrics.operatorVisibleBadPixelTrace.traceCanvasParity.predictionSource, "cpu-final-trace");
  assert.equal(result.metrics.operatorVisibleBadPixelTrace.traceCanvasParity.liveCompositorInputReadbackStatus, "missing");
  assert.equal(result.metrics.operatorVisibleBadPixelClassification.category, "trace-canvas-parity-blocked");
  assert.equal(result.metrics.operatorVisibleBadPixelClassification.stage, "trace-canvas-parity");
  assert.equal(
    result.findings.some((finding) => finding.kind === "operator-visible-bad-pixel-trace-canvas-parity-blocked"),
...
test("static dessert witness classifier blocks color-transfer accusation when trace model diverges from live compositor input", () => {
  const visualGapAnchors = [
    { id: "visual-gap-1", kind: "plate-covered-tile-local-missing", x: 640, y: 342, score: 71, plateDelta: 88, tileLocalDelta: 9 },
  ];
  const badPixelAnchors = [
  ...
  const liveInputParityWithTraceModelMismatch = traceCanvasParityForTest(badPixelAnchors);
  liveInputParityWithTraceModelMismatch.traceModelVsLive = {
    status: "mismatch",
    mismatchAnchors: ["operator-bad-pixel-1"],
    maxDelta: 56,
  };
  liveInputParityWithTraceModelMismatch.anchors = liveInputParityWithTraceModelMismatch.anchors.map((anchor) => ({
    ...anchor,
    predictedRgba8: [222, 218, 208, 255],
    sampledRgba8: [222, 218, 208, 255],
    deltaRgba8: [0, 0, 0, 0],
    maxDelta: 0,
    status: "match",
    cpuFinalTraceRgba8: [199, 188, 177, 255],
    liveCompositorRgba8: [222, 218, 208, 255],
    traceModelVsLiveDeltaRgba8: [23, 30, 31, 0],
    traceModelVsLiveMaxDelta: 31,
  }));
  const result = classifyStaticDessertWitness({
    captures: [
      witnessCapture("final-color", {
      ...
      witnessCapture("operator-visible-bad-pixel-trace", {
        ...
        traceCanvasParity: liveInputParityWithTraceModelMismatch,
      }),
    ],
  });
  ...
  assert.equal(result.metrics.operatorVisibleBadPixelTrace.status, "present");
  assert.equal(result.metrics.operatorVisibleBadPixelTrace.traceCanvasParity.status, "trace-canvas-match");
  assert.equal(result.metrics.operatorVisibleBadPixelTrace.traceCanvasParity.predictionSource, "live-compositor-input-readback");
  assert.equal(result.metrics.operatorVisibleBadPixelTrace.traceCanvasParity.liveCompositorInputReadbackStatus, "present");
  assert.equal(result.metrics.operatorVisibleBadPixelTrace.traceCanvasParity.traceModelVsLive.status, "mismatch");
  assert.equal(result.metrics.operatorVisibleBadPixelClassification.category, "trace-model-live-parity-blocked");
  assert.equal(result.metrics.operatorVisibleBadPixelClassification.stage, "trace-model-vs-live");
  assert.equal(
...
test("static dessert witness classifier attributes matching dark fall-through anchors while keeping mismatched anchors blocked", () => {
  const visualGapAnchors = [
    { id: "visual-gap-1", kind: "plate-covered-tile-local-missing", x: 640, y: 342, score: 71, plateDelta: 88, tileLocalDelta: 9 },
  ];
  const badPixelAnchors = [
  ...
  const mixedParity = traceCanvasParityForTest(badPixelAnchors);
  mixedParity.anchors = mixedParity.anchors.map((anchor) => {
    if (anchor.id !== "operator-bad-pixel-1") {
      return anchor;
    }
    return {
      ...anchor,
      sampledRgba8: [212, 199, 178, 255],
      deltaRgba8: [-25, -25, -23, 0],
      maxDelta: 25,
      status: "mismatch",
    };
  });
  mixedParity.status = "mismatch";
  const result = classifyStaticDessertWitness({
    captures: [
      witnessCapture("final-color", {
        rendererLabel: "tile-local-visible-gaussian-compositor",
      ...
      witnessCapture("operator-visible-bad-pixel-trace", {
        ...
        perPixelFinalColorAccumulation: [
          ...
          {
            status: "present",
            anchorPixel: { id: "operator-bad-pixel-2", x: 591, y: 361 },
            finalColorAccumulation: {
              steps: [{ splatIndex: 22, sourceRole: "foreground", sourceFrontierSupportPixelWeight: 0.04 }],
              outputColor: [0.38, 0.34, 0.27, 1],
              remainingTransmittance: 0,
            },
          },
        ],
        ...
        traceCanvasParity: mixedParity,
      }),
    ],
  });

  const darkAnchor = result.metrics.operatorVisibleBadPixelTrace.anchors.find(
    (anchor) => anchor.kind === "operator-visible-dark-fallthrough",
  );
  assert.equal(result.closeable, false);
  assert.equal(result.metrics.operatorVisibleBadPixelTrace.traceCanvasParity.status, "trace-canvas-mismatch");
  assert.equal(darkAnchor.traceCanvasParityStatus, "match");
  assert.equal(darkAnchor.liveCompositorRgba8.length, 4);
  assert.equal(result.metrics.operatorVisibleBadPixelClassification.status, "blocked");
  assert.equal(result.metrics.operatorVisibleBadPixelClassification.category, "selected-anchor-post-alpha-color-mismatch");
  assert.equal(result.metrics.operatorVisibleBadPixelClassification.classifiedAnchorCount, 1);
  assert.equal(result.metrics.operatorVisibleBadPixelClassification.blockerCount, 1);
});
...
test("visual smoke CLI exposes a static dessert witness batch mode", () => {
  ...
  assert.match(source, /Trace\/canvas parity/);
  assert.match(source, /metrics\.operatorVisibleBadPixelTrace\.traceCanvasParity/);
...
function witnessCapture(id, overrides = {}) {
  ...
  return {
    ...
    pageEvidence: {
      rendererLabel,
      witness: {
        ...(overrides.traceCanvasParity ? { traceCanvasParity: overrides.traceCanvasParity } : {}),
        projection: {
          cropSupport: {
            rimBand: {
...
function traceCanvasParityForTest(anchors, overrides = {}) {
  const status = overrides.status ?? "match";
  const predictedRgba8 = overrides.predictedRgba8 ?? [237, 224, 201, 255];
  const sampledRgba8 = overrides.sampledRgba8 ?? predictedRgba8;
  const deltaRgba8 = overrides.deltaRgba8 ?? [0, 0, 0, 0];
  const maxDelta = overrides.maxDelta ?? 0;
  return {
    observationId: "operator-visible-bad-pixel-trace-test",
    comparisonClass: "exact-route-live-compositor-input-vs-canvas",
    predictionSource: "live-compositor-input-readback",
    liveCompositorInputReadbackStatus: "present",
    ...
    anchors: anchors.map((anchor) => ({
      id: anchor.id,
      anchorPixel: { x: anchor.x, y: anchor.y },
      samplePixel: { x: anchor.x, y: anchor.y },
      predictedRgba8,
      sampledRgba8,
      deltaRgba8,
      maxDelta,
      status,
      predictionSource: "live-compositor-input-readback",
      cpuFinalTraceRgba8: predictedRgba8,
      liveCompositorRgba8: predictedRgba8,
    })),
  };
}
