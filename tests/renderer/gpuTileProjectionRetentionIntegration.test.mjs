...
test("GPU projection/retention builder owns projected, retained, and dropped custody", () => {
  ...
  assert.deepEqual(arena.retainedRecords.map((record) => record.originalId), [101, 102, 103]);
  assert.deepEqual(arena.droppedRecords.map((record) => record.originalId), [100, 104]);
  assert.deepEqual([...arena.tileHeaderU32.slice(0, 4)], [0, 3, 5, 2]);
  assert.deepEqual(arena.scatteredRecords.map((record) => record.originalId), [101, 102, 103]);
});
