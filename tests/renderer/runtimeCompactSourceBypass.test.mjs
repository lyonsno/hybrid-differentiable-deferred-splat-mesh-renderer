...
test("WGSL projected source-frontier route skips CPU streaming retention and visibly owns source construction", () => {
  ...
  assert.match(
    readCompositorInputAnchorSource,
    /const alphaTransferWeight = sourceFrontierAlphaTransferWeight\(\s*pixelCoverageWeight,\s*tileCoverageWeight,\s*tileLocalSupportWeight,\s*sourceFrontierSupportPixelWeight,\s*candidateSourceClassMask,\s*\)/,
