 ...
 *
 * @param {number[][]} binDirs - (G) array of [x,y,z] bin center unit vectors.
 * @param {Int32Array[]} counts - (N) array of Int32Array(G) histograms, parallel
 *   to viewDirs: counts[i] is the histogram for the element whose view direction
 *   is viewDirs[i]. Pass a pre-sliced subset when scoring a subset of elements
 *   (see DirectionalCoverageTracker.scoreCandidateView).
 * @param {number[][]} viewDirs - (N) array of [x,y,z] view direction per element.
 * @returns {number[]} (N) array of per-element max coverage scores in [0, 1].
