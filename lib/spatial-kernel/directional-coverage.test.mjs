...

{
  ...
  // Partial visibility: only element 0 visible from a new direction.
  // Element 0 coverage should increase or stay flat; elements 1-3 unchanged.
  const covBefore = tracker.perElementCoverage;
  tracker.updateFromVisibility([0], positions, [0, 0, 5]);
  const covAfter = tracker.perElementCoverage;

  assert.ok(covAfter[0] >= covBefore[0] - 1e-10,
    `element 0 coverage should not decrease after partial update`);
  for (let i = 1; i < 4; i++) {
    assertClose(covAfter[i], covBefore[i],
      `element ${i} coverage should be unchanged after partial update`);
  }
