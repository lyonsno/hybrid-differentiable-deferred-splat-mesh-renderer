...
- Status: FAIL
- Generated: 2026-06-15T18:54:00.946Z
...
```json
{
  ...
  "statsText": "1280×720 | 0 fps | 94,406 real Scaniverse splats | renderer: compute | sort: gpu-bitonic-cpu-depth-keys | alpha: coverage-aware density 90,551 splats/13 tiles | arena requested: cpu | arena effective: cpu | app frame: 1.7ms | slowest app stage: gpu-sort-refresh 1ms",
  "title": "Deferred Splat+Mesh Renderer",
  "bodyText": "1280×720 | 0 fps | 94,406 real Scaniverse splats | renderer: compute | sort: gpu-bitonic-cpu-depth-keys | alpha: coverage-aware density 90,551 splats/13 tiles | arena requested: cpu | arena effective: cpu | app frame: 1.7ms | slowest app stage: gpu-sort-refresh 1ms",
  "canvas": {
    "width": 1280,
    "height": 720,
    "clientWidth": 1280,
...
```json
{
  "consoleMessages": [
    ...
    {
      "type": "warning",
      "text": "Error while parsing WGSL: :87:31 error: parsed as template list\n        let needSwap = select(a.depth < b.depth, a.depth > b.depth, ascending);\n                              ^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n\n:87:31 note: if this is intended to be a less-than expression then wrap in parentheses\n        let needSwap = select(a.depth < b.depth, a.depth > b.depth, ascending);\n                              ^^^^^^^^^^^^^^^^^\n\n\n - While calling [Device].CreateShaderModule([ShaderModuleDescriptor \"\"tile_sort_shader\"\"]).\n"
    },
    {
      "type": "warning",
      "text": "[Invalid ShaderModule \"tile_sort_shader\"] is invalid due to a previous error.\n - While validating compute stage ([Invalid ShaderModule \"tile_sort_shader\"], entryPoint: \"tile_sort\").\n - While calling [Device].CreateComputePipeline([ComputePipelineDescriptor \"\"tile_sort\"\"]).\n"
    },
    {
      "type": "warning",
      "text": "[Invalid ComputePipeline \"tile_sort\"] is invalid due to a previous error.\n - While encoding [ComputePassEncoder \"tile_sort\"].SetPipeline([Invalid ComputePipeline \"tile_sort\"]).\n - While finishing [CommandEncoder (unlabeled)].\n"
    },
    {
      "type": "warning",
